...
{
	if (PlayerInfo[playerid][pEntrance] != -1)
	{
	    for (new i = 0; i != MAX_ENTRANCES; i ++) if (EntranceData[i][entranceExists] && EntranceData[i][entranceID] == PlayerInfo[playerid][pEntrance] && GetPlayerInterior(playerid) == EntranceData[i][entranceInterior] && GetPlayerVirtualWorld(playerid) == EntranceData[i][entranceWorld])
	        return i;
	}
	return -1;
}
...
CMD:editarentrada(playerid, params[])
{
	...
	if (sscanf(params, "ds[24]S()[128]", id, type, string))
 	{
	 	SendSyntaxMessage(playerid, "/editarentrada [id] [syntax]");
	...
	else if (!strcmp(type, "personalizado", true))
	{
	    ...
	    if (sscanf(string, "d", status))
	        return SendSyntaxMessage(playerid, "/editarentrada [id] [personalizado] [0/1]");
	...
	else if (!strcmp(type, "vw", true))
	{
	    ...
	    if (sscanf(string, "d", worldid))
	        return SendSyntaxMessage(playerid, "/editarentrada [id] [vw] [virtual world]");
	...
	else if (!strcmp(type, "senha", true))
	{
	    ...
	    if (sscanf(string, "s[32]", password))
	        return SendSyntaxMessage(playerid, "/editarentrada [id] [senha] [senha da entrada] (use 'nenhuma' para desabilitar)");
	...
	else if (!strcmp(type, "trancar", true))
	{
	    ...
	    if (sscanf(string, "d", locked))
	        return SendSyntaxMessage(playerid, "/editarentrada [id] [trancar] [trancar 0/1]");
...
CMD:destruirentrada(playerid, params[])
{
	...
	if (sscanf(params, "d", id))
	    return SendSyntaxMessage(playerid, "/destruirentrada [ID]");

	if ((id < 0 || id >= MAX_ENTRANCES) || !EntranceData[id][entranceExists])
	    return SendErrorMessage(playerid, "Você específicou o ID de uma entrada inválida.");

	Entrance_Delete(id);
	SendServerMessage(playerid, "Você destruiu com sucesso a entrada ID: %d.", id);
	return 1;
}
