...
stock Gate_Nearest(playerid, Float: range = 5.0)
{
...
stock gat_OnPlayerConnect(playerid)
{
...
stock gat_OnPlayerDisconnect(playerid)
{
	if(EditingGateID[playerid] != -1) GateData[ EditingGateID[playerid] ][GateEditing] = false;
	return 1;
}

stock gat_OnPlayerEditDynamicObject(playerid, STREAMER_TAG_OBJECT objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
		    ...
		    switch(EditingGateType[playerid])
		    {
				case GATE_STATE_CLOSED:
				{
		           ...
		           // Streamer_SetFloatData(STREAMER_TYPE_3D_TEXT_LABEL, GateData[id][GateLabel], E_STREAMER_Y, y);
					...
					SendServerMessage(playerid, "Posição padrão do portão editado.");
				...
				{
				    SendServerMessage(playerid, "Posição de abertura do portão editado.");
		...
		case EDIT_RESPONSE_CANCEL:
		{
		    ...
		    switch(EditingGateType[playerid])
		    {
				case GATE_STATE_CLOSED:
				{
					...
					GateData[id][GateRot][1] = ry;
					GateData[id][GateRot][2] = rz;
					SendServerMessage(playerid, "Edição da posição padrão do portão foi cancelado.");
				}

				case GATE_STATE_OPEN:
				{
				    SendServerMessage(playerid, "Posição de abertura do portão foi cancelado.");
...
stock gat_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_GATE_PASSWORD)
	{
		...
		if(id == -1) return SendErrorMessage(playerid, "Você não está perto de um portão.");
	...
	if(dialogid == DIALOG_GATE_EDITMENU)
	{
		...
		new id = EditingGateID[playerid];
		if(id == -1) return SendErrorMessage(playerid, "Você não está editando um portão.");
	    if(listitem == 0)
		{
			ToggleGateState(id);
		...
		if(listitem == 2)
		{
		    ...
		    SendServerMessage(playerid, "Editando posição padrão do portão.");
		}
		...
		if(listitem == 3)
		{
		    ...
		    EditingGateType[playerid] = GATE_STATE_OPEN;
		    EditDynamicObject(playerid, GateData[id][GateObject]);
		    SendServerMessage(playerid, "Editando posição de abertura do portão.");
		}
		...
		if(listitem == 4)
		{
		    ...
		    SendServerMessage(playerid, "Você removeu com sucesso o portão ID: %d.", id);
		}
	...
	if(dialogid == DIALOG_GATE_NEWPASSWORD)
	{
	    ...
	    SendServerMessage(playerid, "Você atualizou com sucesso a senha do portão ID: %d.", id);
	    SaveGate(id);
	    ShowEditMenu(playerid, id);
	    return 1;
	}
...
CMD:portao(playerid, params[])
{
	new id = Gate_Nearest(playerid);
	if(id == -1) return SendErrorMessage(playerid, "Você não está perto de um portão.");
	if(GateData[id][GateEditing]) return SendErrorMessage(playerid, "Este portão esta sendo editado, você não pode usa-lo agora.");
	if(GateData[id][GateOpenPos][0] == 0.0 && GateData[id][GateOpenRot][0] == 0.0) return SendErrorMessage(playerid, "Esse portão não pode ser aberto agora.");
...
CMD:criarportao(playerid, params[])
{
	...
	if(EditingGateID[playerid] != -1) return SendErrorMessage(playerid, "Você não pode criar um portão enquanto edita outro.");
	new id = Iter_Free(Gates);
	if(id == -1) return SendErrorMessage(playerid, "Limite de portões alcançado, você não pode adicionar mais.");
	new model, password[GATE_PASS_LEN];
	if(sscanf(params, "iS()["#GATE_PASS_LEN"]", model, password)) return SendSyntaxMessage(playerid, "/criarportao [model id] [senha (opcional)]");
	GateData[id][GateModel] = model;
	GateData[id][GatePassword] = password;
	...
	EditingGateType[playerid] = GATE_STATE_CLOSED;
	EditDynamicObject(playerid, GateData[id][GateObject]);
	SendServerMessage(playerid, "Portão criado com sucesso, agora você pode edita-lo.");
	return 1;
}
...
CMD:editarportao(playerid, params[])
{
	...
	if(EditingGateID[playerid] != -1) return SendErrorMessage(playerid, "Você já está editando um portão.");
	new id;
	sscanf(params, "I(-2)", id);
	if(id == -2) id = Gate_Nearest(playerid);
	if(id == -1) return SendErrorMessage(playerid, "Você não está perto de um portão.");
	if(GateData[id][GateEditing]) return SendErrorMessage(playerid, "Esse portão já está sendo editado.");
	if(!IsPlayerInRangeOfPoint(playerid, 20.0, GateData[id][GatePos][0], GateData[id][GatePos][1], GateData[id][GatePos][2])) return SendErrorMessage(playerid, "{FFFFFF}You're not near the gate you want to edit.");
	GateData[id][GateEditing] = true;
	EditingGateID[playerid] = id;
	ShowEditMenu(playerid, id);
	return 1;
